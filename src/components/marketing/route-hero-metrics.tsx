...
export function RouteHeroMetrics({ rate, days, freqWeekly, mode }: Props) {
  const stats = [
      ...
      <div className="grid min-w-0 grid-cols-2 gap-4 sm:gap-6 lg:grid-cols-4 lg:gap-8">
        {stats.map((stat) => (
          <div key={stat.l} className="min-w-0 text-center">
            ...
            <div
              className="f-display tabular mt-1.5 text-xl sm:mt-2 sm:text-2xl md:text-3xl"
              style={stat.l === "INDICATIVE FROM" ? { color: "var(--cargo)" } : undefined}
            >
              {stat.v}
            </div>
          </div>
