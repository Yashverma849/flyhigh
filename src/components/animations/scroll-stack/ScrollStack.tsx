...
export default function ScrollStack({
  ...
  const measureLayoutPositions = useCallback(() => {
    ...
    if (useWindowScroll) {
      ...
      const endElement = scroller.querySelector(".scroll-stack-end");
      endLayoutTopRef.current = endElement ? (endElement as HTMLElement).offsetTop : 0;
    }
  }, [useWindowScroll]);
  ...
  const getElementOffset = useCallback(
    (element: HTMLElement, index?: number) => {
      if (index !== undefined && cardLayoutTopsRef.current[index] !== undefined) {
        return cardLayoutTopsRef.current[index]!;
      }
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const firstCardTop = getElementOffset(firstCard, 0);
    const stackActiveStart = firstCardTop - stackPositionPx;
    const lastIndex = cards.length - 1;
    const lastCard = cards[lastIndex]!;
    const lastCardTop = getElementOffset(lastCard, lastIndex);
    const lastCardHeight = lastCard.offsetHeight;
    // Release before the next page section can enter the viewport while cards are still pinned.
    const stackReleaseScroll = lastCardTop + lastCardHeight - containerHeight;

    if (scrollTop < stackActiveStart || scrollTop >= stackReleaseScroll) {
    ...
    cards.forEach((card, i) => {
      ...
      const pinEnd = stackReleaseScroll - 1;
  ...
  const setupLenis = useCallback(() => {
    ...
    return lenis;
  }, [handleScroll, useWindowScroll, measureLayoutPositions, updateCardTransforms]);
  ...
  useLayoutEffect(() => {
    ...
    const resizeCleanup = setupLenis();
    updateCardTransforms();
